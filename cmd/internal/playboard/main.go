...
func (*Command) Name() string     { return "playboard" }
func (*Command) Synopsis() string { return "Play Tak from the command line with preset PTN" }
func (*Command) Usage() string {
	return `playboard

Play Tak on the command-line, against a human or AI, with preset PTN.
`
}
...
func (c *Command) SetFlags(flags *flag.FlagSet) {
	...
	flags.StringVar(&c.ptn, "ptn", "", "read initial state from ptn")
...
func (c *Command) Execute(ctx context.Context, flag *flag.FlagSet, _ ...interface{}) subcommands.ExitStatus {
	...
	parsed, _ := ptn.ParseFile(c.ptn)
	pos, _ := parsed.PositionAtMove(0, tak.NoColor)
	st := &cli.CLI{
		Config: pos.Config(),
		Out:    os.Stdout,
